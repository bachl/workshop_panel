...
* Das `lmtest` Paket [@R-lmtest] ist kompatibel mit Modellen aus `plm`. Es stellt zahlreiche robuste Standardfehler zur Verfügung.
...
* Weiter wollen wir dieses Thema hier nicht vertiefen. Ich empfehle unter anderem @kingHowRobustStandard2015 zur kritischen Lektüre.
...
> In many applications the whole point of using panel data is to allow for $a_i$ to be arbitrarily correlated with the $x_{it}$. A fixed effects analysis achieves this purpose explicitly. --- @wooldridge10, p. 300
...
* Das *fixed effects* Modell kann keine Mermale der Einheiten (Personen) als Prädiktoren berücksichtigen, da die gesamten einheiten(personen)spezifischen Unterschiede bereits durch die *fixed effects* erklärt werden.
...
2) Spezifiziere, schätze und interpretiere ein eigenes *fixed effects* Modell mit Daten aus dem Beipsieldatensatz. Nutze dabei alle Techniken (unterschiedliche Spezifikation, Standardfehler, Moderation, mehrere Prädiktoren), die du ausprobieren und zu denen du ggf. Fragen stellen willst.
